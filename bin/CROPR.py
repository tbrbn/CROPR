...
class MainWindow:
    def __init__(self, root):
        ...
        self.checkbox_views_var = IntVar()
        self.checkbox_alfa_var = IntVar()
        ...
        self.checkbox_views = Checkbutton(self.frame_buttons, variable=self.checkbox_views_var,
                                        text="All views", onvalue = 1, offvalue = 0, command=lambda: self.checkbox())
        self.checkbox_alfa = Checkbutton(self.frame_buttons, variable=self.checkbox_alfa_var,
                                          text="Transparent", onvalue=1, offvalue=0)
        self.button.pack(side=LEFT, padx=3, pady=3)
        self.button_dir.pack(side=LEFT, padx=3, pady=3)
        self.checkbox_views.pack(side=LEFT, padx=3, pady=3)
        self.checkbox_alfa.pack(side=LEFT, padx=3, pady=3)
    ...
    def file_opener(self):
        var_v = self.checkbox_views_var.get()
        var_a = self.checkbox_alfa_var.get()
        self.reset_canvas()
        self.checkbox_views_var.set(var_v)
        self.checkbox_alfa_var.set(var_a)
        a = askopenfile()
        if a is not None:
...
class Lines:
    def __init__(self, mainwindow, master, canvas):
        ...
        if self.main_window.checkbox_views_var.get() == 0:
            self.view_text = "Side\n"
        else:
    ...
    def on_click(self, event):
        ...
        self.update_text(self.i)
        self.main_window.checkbox_views['state'] = 'disabled'
        x0, y0 = self.canvas.canvasx(0), self.canvas.canvasy(0)
        wx0, wy0 = event.x + x0, event.y + y0
        ...
        if len(self.clicks) == 2:
            ...
            if self.ground_mode:
                ...
                self.i = self.i + 1
                self.update_text(self.i)
        if (self.i == 4 and self.main_window.checkbox_views_var.get() == 0)\
                or (self.i == 6 and self.main_window.checkbox_views_var.get() == 1):
            self.master.unbind('<Escape>')
            if len(self.clicks) == 1:
                self.autoscale_finish()
    ...
    def autoscale_finish(self):
        ...
        autoScaler = AutoScaler(self.main_window.images_unscaled, self.main_window.img_dir,
                                ...
                                self.on_off, self.main_window.checkbox_views_var.get(),
                                self.main_window.checkbox_alfa_var.get())
...
class AutoScaler:
    def __init__(self, images_unscaled, img_dir, img_name, on_off, all_views, alfa):
        self.views = ["", "", "", "", "", ""]
        self.on_off = on_off
        self.images_unscaled = images_unscaled
        if alfa == 1:
            self.images_unscaled = self.make_transparent(images_unscaled)
        else:
            self.images_unscaled = images_unscaled
        if all_views == 0:
            ## order is Side - Front - Top -Rear
            h_all = min(self.images_unscaled[0].size[1], self.images_unscaled[1].size[1], self.images_unscaled[3].size[1])
            l_all = min(self.images_unscaled[0].size[0], self.images_unscaled[2].size[0])
            w_all = min(self.images_unscaled[1].size[0], self.images_unscaled[3].size[0], self.images_unscaled[2].size[1])
            # print(maxsize_a, maxsize_b, maxsize_c)

            if on_off[0] == 1:
                #images_unscaled[0].thumbnail((maxsize_b, maxsize_a))
                a = self.images_unscaled[0].resize((l_all, h_all))
            ...
            if on_off[1] == 1:
                ...
                a = self.images_unscaled[1].resize((w_all, h_all))
                a.save(img_dir + img_name + "_front.png", "PNG")
                self.views[1] = img_name + "_front.png"

            if on_off[2] == 1:
                #images_unscaled[2].thumbnail((maxsize_b, maxsize_c))
                a = self.images_unscaled[2].resize((l_all, w_all))
                a.save(img_dir + img_name + "_top.png", "PNG")
                self.views[2] = img_name + "_top.png"

            if on_off[3] == 1:
                #images_unscaled[3].thumbnail((maxsize_c, maxsize_a))
                a = self.images_unscaled[3].resize((w_all, h_all))
                a.save(img_dir + img_name + "_rear.png", "PNG")
                self.views[3] = img_name + "_rear.png"
        else:
            ## order is Right - Left - Front - Rear - Top - Bottom
            h_all = min(self.images_unscaled[0].size[1], self.images_unscaled[1].size[1], self.images_unscaled[2].size[1], self.images_unscaled[3].size[1])
            l_all = min(self.images_unscaled[0].size[0], self.images_unscaled[1].size[0], self.images_unscaled[4].size[0], self.images_unscaled[5].size[0])
            w_all = min(self.images_unscaled[2].size[0], self.images_unscaled[3].size[0], self.images_unscaled[4].size[1], self.images_unscaled[5].size[1])
            # print(maxsize_a, maxsize_b, maxsize_c)
            ...
            if on_off[0] == 1:
                ...
                self.views[0] = img_name + "_right.png"

            if on_off[1] == 1:
                # images_unscaled[1].thumbnail((maxsize_c, maxsize_a))
                a = self.images_unscaled[1].resize((l_all, h_all))
                a.save(img_dir + img_name + "_left.png", "PNG")
                self.views[1] = img_name + "_left.png"

            if on_off[2] == 1:
                # images_unscaled[2].thumbnail((maxsize_b, maxsize_c))
                a = self.images_unscaled[2].resize((w_all, h_all))
                a.save(img_dir + img_name + "_front.png", "PNG")
            ...
            if on_off[4] == 1:
                a = self.images_unscaled[4].resize((l_all, w_all))
                a.save(img_dir + img_name + "_top.png", "PNG")
                self.views[4] = img_name + "_top.png"

            if on_off[5] == 1:
                a = self.images_unscaled[5].resize((l_all, w_all))
    ...
    def make_transparent(self, img_list):
        output = []
        for i in img_list:
            element = []
            a = i.convert("RGBA")
            data = a.getdata()
            for item in data:
                if all(x > 235 for x in item[0:3]):
                    element.append((255, 255, 255, 0))
                else:
                    element.append(item)
            a.putdata(element)
            output.append(a)
